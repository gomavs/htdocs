...
$id = 1;
$query = $db->prepare("SELECT * FROM workdata WHERE assignedTo = ?  AND status = 0 ORDER BY id ASC");
$query->bind_param("i", $id);
$query->execute();
$result = $query->get_result();
while (($row = $result->fetch_object()) !== NULL) {
	$query1 = $db->prepare("SELECT * FROM workorder WHERE id = ?");
	$query1->bind_param("i", $row->workOrderId);
	$query1->execute();
	$result1 = $query1->get_result();
	$row1 = $result1->fetch_assoc();
	$workRequestId = $row1['workRequestId'];
	$startDate = date("F j, Y" ,$row1['startDate']);
	$dueDate = date("F j, Y", $row1['dueDate']);
	$timeEstimate = secondsToTime($row1['timeEstimate']);
	$notes = $row1['notes'];
	...
	$assignedTo = assignment($row->workOrderId);
