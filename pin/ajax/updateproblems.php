...
if(isset($_POST['problemId'])){
	...
	foreach($delete_diff as $check){
		$check = intval($check);
		$query = $db->prepare("UPDATE problems SET active = 0 WHERE problemType = ? AND problemId = ? AND itemId = ?");
	...
	foreach($insert_diff as $value){
		...
		if(in_array($value, $data3)){
			$data[] = $value;
			$query = $db->prepare("UPDATE problems SET active = 1 WHERE problemType = ? AND problemId = ? AND itemId = ?");
			$query->bind_param("iii", $problemType, $problemId, $value);
			$query->execute();
		}else{
			$query = $db->prepare("INSERT INTO problems (problemType, problemId,itemId, active) VALUES (?, ?, ?, ?)");
